...
class ManualAddDialog(tk.Toplevel):
    def __init__(self, parent: tk.Tk, app: App, bag: BagState):
        ...
        self.tree = ttk.Treeview(self, columns=("name", "price", "cost", "stock", "status"), show="headings", height=14)
        for c, t, w in [
            ("name", "游戏名", 360),
            ("price", "售价", 90),
            ("cost", "成本", 90),
    ...
    def _refresh_cost_check(self):
        ...
        msg = f"成本校验：当前商品成本 {it.cost:.2f} 元，{LEVEL_NAME[lvl]}范围 {r.lo}~{r.hi}：{'是' if ok else '否'}"
        self.lbl_cost_check.configure(text=msg, foreground=("#1f6f3c" if ok else "#8a1c1c"))
    ...
    def confirm(self):
        ...
        it = next((x for x in self.app.catalog if x.id == it_id), None)
        if not it:
            messagebox.showerror("异常", "商品不存在。")
            return

        r = self.bag.cfg.cost_ranges[lvl]
        if not (r.lo <= it.cost <= r.hi):
            messagebox.showerror("校验失败", f"该商品成本 {it.cost:.2f} 不在 {LEVEL_NAME[lvl]} 成本范围 {r.lo}~{r.hi} 内。")
            return
        ...
        messagebox.showinfo("已添加", f"已添加：{it.name} → {LEVEL_NAME[lvl]}（来源：手动添加）")
        self.destroy()
